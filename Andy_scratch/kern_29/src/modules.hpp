...
#pragma HLS loop_tripcount min=1 max=512
		for (int j=0; j < config_t::NUM_CHANNEL / config_t::VEC_WIDTH; j++) { // each row
			...
			for (int k=0; k < config_t::VEC_WIDTH; k++) {
				...
				out_1[k%config_t::UNROLL_FACTOR].write(read);
				out_2[k%config_t::UNROLL_FACTOR].write(read);
			}
...
#pragma HLS dataflow
	...
	const static int fifo_depth = (3*config_t::NUM_CHANNEL/config_t::UNROLL_FACTOR)+1;
#pragma HLS stream variable=in_compute depth=fifo_depth
#pragma HLS stream variable=in_compute_y depth=fifo_depth

	//	 save buffer
	layernorm_save_data<config_t>(in, n_pipe1, in_compute, in_compute_y, mean_pipe1, mean_pipe2);

	// compute var
	layernorm_compute_var<config_t>(n_pipe1, n_pipe2, in_compute, in_sqrt, mean_pipe1);

	// square root
	layernorm_sqrt_alg_based<config_t>(n_pipe2, n_pipe3, in_sqrt, in_compute_y_factor);

	// add bias
	layernorm_compute_y<config_t>(n_pipe3, n_pipe4, bias, in_compute_y_factor, in_compute_y, in_write, mean_pipe2);
