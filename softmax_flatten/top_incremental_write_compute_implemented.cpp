	...
	for (int batch = 0; batch < num_batches; batch++) {
		...
		for (int iter = 0; iter < unrolled_iterations; iter++) {
			...
			for (int num = 0; num < config_t::UNROLL_FACTOR; num++) {
				...
				out_temp.range(config_t::QUANT_DATA_WIDTH*(num+1)-1, config_t::QUANT_DATA_WIDTH*num) = result;
			}
			out.write(out_temp);
		}
	}
...
#pragma HLS dataflow
	...
	static hls::stream<ap_int<config_t::IN_DATA_WIDTH*config_t::UNROLL_FACTOR> > save_data_out;
	...
	static hls::stream<ap_int<config_t::QUANT_DATA_WIDTH*config_t::UNROLL_FACTOR> > quantized_exponential_out;
	static hls::stream<ap_uint<32> > compute_softmax_from_exponents_num_unrolled_iter;
	static hls::stream<ap_int<config_t::OUT_DATA_WIDTH*config_t::UNROLL_FACTOR> > compute_softmax_from_exponents_out;
